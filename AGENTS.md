...
**Key rule:** When docs contradict code or CI, the code wins. Documentation in `docs/` describes the **target** system, not always the current implementation.
...
**What's NOT yet real:**
- Some advanced desktop workflows such as fully interactive exec and richer port-forward session management
...
- **`apps/web/AGENTS.md`** — SvelteKit frontend: Svelte 5 patterns, routing, testing
...
2. **Run local validations:** `cargo fmt && cargo clippy && cargo test` for Rust; `pnpm -C apps/web test && pnpm -C apps/web e2e` for frontend
...
6. **After a completed, validated change set:** commit it, push upstream, then create a release tag matching `v*` so `.github/workflows/release.yml` runs
