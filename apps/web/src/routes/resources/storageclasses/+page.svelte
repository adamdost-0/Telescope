<script lang="ts">
  ...
  const GVK = 'storage.k8s.io/v1/StorageClass';
  const PAGE_TITLE = 'Storage Classes';
  ...
  const columns = [
    ...
    { key: 'provisioner', label: 'Provisioner', extract: (c: any) => c?.provisioner ?? '' },
    { key: 'reclaim-policy', label: 'Reclaim Policy', extract: (c: any) => c?.reclaimPolicy ?? 'Delete' },
    { key: 'volume-binding-mode', label: 'Volume Binding Mode', extract: (c: any) => c?.volumeBindingMode ?? 'Immediate' },
    { key: 'default', label: 'Default', extract: (c: any) => {
      const annotations = c?.metadata?.annotations ?? {};
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No storage classes found." hrefFn={(entry) => `/resources/storageclasses/${entry.namespace || '_cluster'}/${entry.name}`} />
  {/if}
</div>
