<script lang="ts">
  ...
  const GVK = 'apps/v1/ReplicaSet';
  const PAGE_TITLE = 'ReplicaSets';
  ...
  const columns = [
    ...
    { key: 'desired', label: 'Desired', extract: (c: any) => String(c?.spec?.replicas ?? 0) },
    { key: 'current', label: 'Current', extract: (c: any) => String(c?.status?.replicas ?? 0) },
    { key: 'ready', label: 'Ready', extract: (c: any) => String(c?.status?.readyReplicas ?? 0) },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No replica sets found in this namespace." hrefFn={(entry) => `/resources/replicasets/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
