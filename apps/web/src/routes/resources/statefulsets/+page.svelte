<script lang="ts">
  ...
  const GVK = 'apps/v1/StatefulSet';
  const PAGE_TITLE = 'StatefulSets';
  ...
  const columns = [
    ...
    { key: 'ready', label: 'Ready', extract: (c: any) => {
      const ready = c?.status?.readyReplicas ?? 0;
      const desired = c?.spec?.replicas ?? 0;
      return `${ready}/${desired}`;
    }},
    { key: 'current', label: 'Current', extract: (c: any) => String(c?.status?.currentReplicas ?? 0) },
    { key: 'service', label: 'Service', extract: (c: any) => c?.spec?.serviceName ?? 'N/A' },
    { key: 'pod-management', label: 'Pod Mgmt', extract: (c: any) => c?.spec?.podManagementPolicy ?? 'OrderedReady' },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable
      resources={filtered}
      {columns}
      emptyMessage="No statefulsets found in this namespace."
      hrefFn={(entry) => `/resources/statefulsets/${entry.namespace}/${entry.name}`}
    />
  {/if}
</div>
