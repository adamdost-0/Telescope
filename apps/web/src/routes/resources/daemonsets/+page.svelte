<script lang="ts">
  ...
  const GVK = 'apps/v1/DaemonSet';
  const PAGE_TITLE = 'DaemonSets';
  ...
  function formatImages(containers: any[]): string {
    if (!containers || containers.length === 0) return '<none>';
    return containers.map((container: any) => container?.image ?? '').filter(Boolean).join(', ') || '<none>';
  }

  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '24%' },
    { key: 'ready', label: 'Ready', extract: (c: any) => {
      const ready = c?.status?.numberReady ?? 0;
      const desired = c?.status?.desiredNumberScheduled ?? 0;
      return `${ready}/${desired}`;
    }},
    { key: 'current', label: 'Current', extract: (c: any) => String(c?.status?.currentNumberScheduled ?? 0) },
    { key: 'up-to-date', label: 'Up-to-date', extract: (c: any) => String(c?.status?.updatedNumberScheduled ?? 0) },
    { key: 'available', label: 'Available', extract: (c: any) => String(c?.status?.numberAvailable ?? 0) },
    { key: 'age', label: 'Age', extract: (c: any) => {
      const ts = c?.metadata?.creationTimestamp;
      return ts ? formatAge(ts) : 'Unknown';
    }},
    { key: 'images', label: 'Images', extract: (c: any) => formatImages(c?.spec?.template?.spec?.containers) },
  ];
...
<div class="resource-page">
  <header>
    ...
    <div class="controls">
      ...
      <button
        ...
        aria-label="Refresh {PAGE_TITLE.toLowerCase()}"
      >
  ...
  {:else}
    ...
    <ResourceTable
      resources={filtered}
      {columns}
      emptyMessage="No daemonsets found in this namespace."
      hrefFn={(entry) => `/resources/daemonsets/${entry.namespace}/${entry.name}`}
    />
  {/if}
</div>
