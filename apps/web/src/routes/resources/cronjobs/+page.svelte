<script lang="ts">
  ...
  const GVK = 'batch/v1/CronJob';
  const PAGE_TITLE = 'CronJobs';
  ...
  function formatLastScheduled(timestamp?: string): string {
    if (!timestamp) return 'Never';
    return formatAge(timestamp);
  }

  function getSuspendLabel(cronJob: any): string {
    return cronJob?.spec?.suspend ? 'Yes' : 'No';
  }

  function getSuspendColor(cronJob: any): string | null {
    return cronJob?.spec?.suspend ? '#facc15' : '#4ade80';
  }

  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '28%' },
    { key: 'schedule', label: 'Schedule', extract: (c: any) => c?.spec?.schedule ?? 'N/A' },
    { key: 'suspend', label: 'Suspend', extract: (c: any) => getSuspendLabel(c), colorFn: (c: any) => getSuspendColor(c) },
    { key: 'active', label: 'Active Jobs', extract: (c: any) => String(c?.status?.active?.length ?? 0) },
    { key: 'last-scheduled', label: 'Last Scheduled', extract: (c: any) => formatLastScheduled(c?.status?.lastScheduleTime) },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No cronjobs found in this namespace." hrefFn={(entry) => `/resources/cronjobs/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
