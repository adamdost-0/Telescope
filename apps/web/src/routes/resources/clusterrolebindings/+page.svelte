<script lang="ts">
  ...
  const GVK = 'rbac.authorization.k8s.io/v1/ClusterRoleBinding';
  const PAGE_TITLE = 'ClusterRoleBindings';
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No cluster role bindings found." hrefFn={(entry) => `/resources/clusterrolebindings/_cluster/${entry.name}`} />
  {/if}
</div>
