<script lang="ts">
  ...
  const GVK = 'v1/PersistentVolumeClaim';
  const PAGE_TITLE = 'Persistent Volume Claims';
  ...
  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '24%' },
    {
      key: 'status',
      label: 'Status',
      extract: (c: any) => c?.status?.phase ?? 'Unknown',
      colorFn: (c: any) => c?.status?.phase === 'Bound' ? 'var(--success)' : c?.status?.phase === 'Pending' ? 'var(--warning)' : null,
    },
    { key: 'volume', label: 'Volume', extract: (c: any) => c?.spec?.volumeName ?? 'Unbound' },
    { key: 'capacity', label: 'Capacity', extract: (c: any) => c?.status?.capacity?.storage ?? 'N/A' },
    { key: 'requested', label: 'Requested', extract: (c: any) => c?.spec?.resources?.requests?.storage ?? 'N/A' },
    { key: 'access', label: 'Access Modes', extract: (c: any) => (c?.spec?.accessModes ?? []).join(', ') || 'N/A' },
    { key: 'class', label: 'Storage Class', extract: (c: any) => c?.spec?.storageClassName ?? 'N/A' },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable
      resources={filtered}
      {columns}
      emptyMessage="No persistent volume claims found in this namespace."
      hrefFn={(entry) => `/resources/pvcs/${entry.namespace}/${entry.name}`}
    />
  {/if}
</div>
