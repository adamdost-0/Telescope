<script lang="ts">
  ...
  const GVK = 'networking.k8s.io/v1/Ingress';
  const PAGE_TITLE = 'Ingresses';
  ...
  function formatHosts(rules: any[]): string {
  ...
  function formatAddress(ingress: any): string {
    const addresses = ingress?.status?.loadBalancer?.ingress ?? [];
    if (!addresses.length) return 'Pending';
    return addresses
      .map((address: any) => address?.ip ?? address?.hostname ?? '')
      .filter(Boolean)
      .join(', ') || 'Pending';
  }
  ...
  const columns = [
    ...
    { key: 'class', label: 'Class', extract: (c: any) => c?.spec?.ingressClassName ?? 'N/A' },
    { key: 'hosts', label: 'Hosts', extract: (c: any) => formatHosts(c?.spec?.rules) },
    { key: 'address', label: 'Address', extract: (c: any) => formatAddress(c) },
    { key: 'tls', label: 'TLS', extract: (c: any) => c?.spec?.tls?.length ? `${c.spec.tls.length}` : '0' },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable
      resources={filtered}
      {columns}
      emptyMessage="No ingresses found in this namespace."
      hrefFn={(entry) => `/resources/ingresses/${entry.namespace}/${entry.name}`}
    />
  {/if}
</div>
