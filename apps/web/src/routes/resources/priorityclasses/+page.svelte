<script lang="ts">
  ...
  const GVK = 'scheduling.k8s.io/v1/PriorityClass';
  const PAGE_TITLE = 'Priority Classes';
  ...
  const columns = [
    ...
    { key: 'value', label: 'Value', extract: (c: any) => String(c?.value ?? 0) },
    { key: 'globalDefault', label: 'Global Default', extract: (c: any) => c?.globalDefault ? 'Yes' : 'No' },
    { key: 'preemption', label: 'Preemption Policy', extract: (c: any) => c?.preemptionPolicy ?? 'PreemptLowerPriority' },
  ...
  async function loadResources() {
    ...
    error = null;
    try {
      resources = await getResources(GVK);
      lastUpdated = new Date();
      lastUpdatedText = formatTimestamp();
    } catch (e) {
      error = e instanceof Error ? e.message : `Failed to load priority classes`;
      resources = [];
    } finally {
      loading = false;
      refreshing = false;
...
<div class="resource-page">
  <header>
    ...
    <div class="controls">
      ...
      {/if}
      <button type="button" onclick={loadResources} disabled={refreshing} class:spinning={refreshing} aria-label="Refresh priority classes">
  ...
  {:else if error}
    <div role="alert" aria-live="polite" class="error-container">
      <p class="error">Failed to load priority classes. Check cluster connection and try again.</p>
      {#if error !== 'Failed to load priority classes'}
  ...
  {:else}
    ...
    <p class="count">{filterQuery ? `${filtered.length} of ${resources.length}` : resources.length} priority classes</p>
    <ResourceTable resources={filtered} {columns} emptyMessage="No priority classes found." hrefFn={(entry) => `/resources/priorityclasses/_cluster/${entry.name}`} />
  {/if}
</div>
