<script lang="ts">
  ...
  const GVK = 'policy/v1/PodDisruptionBudget';
  const PAGE_TITLE = 'Pod Disruption Budgets';
  ...
  function getMinAvailableOrMaxUnavailable(pdb: any): string {
    if (pdb?.spec?.minAvailable != null) return `min: ${pdb.spec.minAvailable}`;
    if (pdb?.spec?.maxUnavailable != null) return `max unavail: ${pdb.spec.maxUnavailable}`;
    return 'N/A';
  }
  ...
  const columns = [
    ...
    { key: 'budget', label: 'Min Available / Max Unavailable', extract: (c: any) => getMinAvailableOrMaxUnavailable(c) },
    { key: 'healthy', label: 'Current Healthy', extract: (c: any) => String(c?.status?.currentHealthy ?? 0) },
    { key: 'allowed', label: 'Disruptions Allowed', extract: (c: any) => String(c?.status?.disruptionsAllowed ?? 0) },
  ...
  async function loadResources() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : `Failed to load PDBs`;
      resources = [];
    } finally {
      loading = false;
      refreshing = false;
...
<div class="resource-page">
  <header>
    ...
    <div class="controls">
      ...
      {/if}
      <button type="button" onclick={loadResources} disabled={refreshing} class:spinning={refreshing} aria-label="Refresh PDBs">
  ...
  {:else if error}
    <div role="alert" aria-live="polite" class="error-container">
      <p class="error">Failed to load PDBs. Check cluster connection and try again.</p>
      {#if error !== 'Failed to load PDBs'}
  ...
  {:else}
    ...
    <p class="count">{filterQuery ? `${filtered.length} of ${resources.length}` : resources.length} PDBs</p>
    <ResourceTable resources={filtered} {columns} emptyMessage="No pod disruption budgets found in this namespace." hrefFn={(entry) => `/resources/poddisruptionbudgets/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
