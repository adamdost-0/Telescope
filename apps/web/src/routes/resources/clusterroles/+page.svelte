<script lang="ts">
  ...
  const GVK = 'rbac.authorization.k8s.io/v1/ClusterRole';
  const PAGE_TITLE = 'ClusterRoles';

  function formatCreationTimestamp(timestamp: string): string {
    return new Date(timestamp).toLocaleString();
  }

  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '50%' },
    { key: 'created', label: 'Created', extract: (c: any) => {
      const ts = c?.metadata?.creationTimestamp;
      return ts ? formatCreationTimestamp(ts) : 'Unknown';
    }},
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No cluster roles found." hrefFn={(entry) => `/resources/clusterroles/_cluster/${entry.name}`} />
  {/if}
</div>
