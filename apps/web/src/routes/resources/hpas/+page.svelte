<script lang="ts">
  ...
  const GVK = 'autoscaling/v2/HorizontalPodAutoscaler';
  const PAGE_TITLE = 'Horizontal Pod Autoscalers';
  ...
  function getScaleTargetRef(hpa: any): string {
  ...
  const columns = [
    ...
    { key: 'reference', label: 'Reference', extract: (c: any) => getScaleTargetRef(c) },
    { key: 'minmax', label: 'Min/Max Replicas', extract: (c: any) => `${c?.spec?.minReplicas ?? 1}/${c?.spec?.maxReplicas ?? 'N/A'}` },
    { key: 'current', label: 'Current Replicas', extract: (c: any) => String(c?.status?.currentReplicas ?? 0) },
  ...
  async function loadResources() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : `Failed to load HPAs`;
      resources = [];
    } finally {
      loading = false;
      refreshing = false;
...
<div class="resource-page">
  <header>
    ...
    <div class="controls">
      ...
      {/if}
      <button type="button" onclick={loadResources} disabled={refreshing} class:spinning={refreshing} aria-label="Refresh HPAs">
  ...
  {:else if error}
    <div role="alert" aria-live="polite" class="error-container">
      <p class="error">Failed to load HPAs. Check cluster connection and try again.</p>
      {#if error !== 'Failed to load HPAs'}
  ...
  {:else}
    ...
    <p class="count">{filterQuery ? `${filtered.length} of ${resources.length}` : resources.length} HPAs</p>
    <ResourceTable resources={filtered} {columns} emptyMessage="No HPAs found in this namespace." hrefFn={(entry) => `/resources/hpas/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
