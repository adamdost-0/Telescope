<script lang="ts">
  ...
  const GVK = 'v1/ConfigMap';
  const PAGE_TITLE = 'ConfigMaps';
  ...
  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '40%' },
    { key: 'keys', label: 'Keys', extract: (c: any) => {
      const data = c?.data ?? {};
      const keys = Object.keys(data);
      return `${keys.length}`;
    }},
    { key: 'key-names', label: 'Key Names', extract: (c: any) => {
      const data = c?.data ?? {};
      const keys = Object.keys(data);
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No configmaps found in this namespace." hrefFn={(entry) => `/resources/configmaps/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
