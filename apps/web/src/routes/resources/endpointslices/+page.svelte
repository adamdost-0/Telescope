<script lang="ts">
  ...
  const GVK = 'discovery.k8s.io/v1/EndpointSlice';
  const PAGE_TITLE = 'EndpointSlices';
  ...
  function formatPorts(ports: any[] | undefined): string {
    ...
    return ports.map((port: any) => {
      ...
      return `${name}${value}/${protocol}`;
    }).join(', ');
  }

  const columns = [
    { key: 'name', label: 'Name', extract: (c: any) => c?.metadata?.name ?? '', width: '22%' },
    { key: 'namespace', label: 'Namespace', extract: (c: any) => c?.metadata?.namespace ?? 'default' },
    { key: 'address-type', label: 'Address Type', extract: (c: any) => c?.addressType ?? 'Unknown' },
    { key: 'ports', label: 'Ports', extract: (c: any) => formatPorts(c?.ports) },
    { key: 'endpoints-count', label: 'Endpoints count', extract: (c: any) => String(c?.endpoints?.length ?? 0) },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No endpoint slices found in this namespace." hrefFn={(entry) => `/resources/endpointslices/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
