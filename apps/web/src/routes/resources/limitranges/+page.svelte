<script lang="ts">
  ...
  const GVK = 'v1/LimitRange';
  const PAGE_TITLE = 'LimitRanges';
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable
      resources={filtered}
      {columns}
      emptyMessage="No limitranges found in this namespace."
      hrefFn={(entry) => `/resources/limitranges/${entry.namespace}/${entry.name}`}
    />
  {/if}
</div>
