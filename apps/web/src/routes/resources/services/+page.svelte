<script lang="ts">
  ...
  const columns = [
    ...
    { key: 'type', label: 'Type', extract: (c: any) => c?.spec?.type ?? 'ClusterIP' },
    { key: 'cluster-ip', label: 'Cluster-IP', extract: (c: any) => c?.spec?.clusterIP ?? '<none>' },
    { key: 'ports', label: 'Ports', extract: (c: any) => formatPorts(c?.spec?.ports) },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No services found in this namespace." hrefFn={(entry) => `/resources/services/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
