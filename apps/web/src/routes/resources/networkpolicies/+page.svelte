<script lang="ts">
  ...
  const GVK = 'networking.k8s.io/v1/NetworkPolicy';
  const PAGE_TITLE = 'NetworkPolicies';
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No network policies found in this namespace." hrefFn={(entry) => `/resources/networkpolicies/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
