<script lang="ts">
  ...
  const columns = [
    ...
    { key: 'up-to-date', label: 'Up-to-date', extract: (c: any) => String(c?.status?.updatedReplicas ?? 0) },
    { key: 'available', label: 'Available', extract: (c: any) => String(c?.status?.availableReplicas ?? 0) },
    { key: 'age', label: 'Age', extract: (c: any) => {
      const ts = c?.metadata?.creationTimestamp;
      return ts ? formatAge(ts) : 'Unknown';
    }},
    { key: 'images', label: 'Images', extract: (c: any) => {
      const containers = c?.spec?.template?.spec?.containers ?? [];
      return containers.map((ctr: any) => ctr.image ?? '').join(', ');
    }},
  ];
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable {resources} {columns} emptyMessage="No deployments found in this namespace." />
  {/if}
</div>
