<script lang="ts">
  ...
  function getJobStatus(job: any): string {
    if (job?.spec?.suspend) return 'Suspended';
    if ((job?.status?.failed ?? 0) > 0) return 'Failed';

    const desired = job?.spec?.completions ?? 1;
    const succeeded = job?.status?.succeeded ?? 0;
    if (job?.status?.completionTime || succeeded >= desired) return 'Complete';
    if ((job?.status?.active ?? 0) > 0) return 'Running';
    if (job?.status?.startTime) return 'Starting';

    return 'Pending';
  }

  function getJobStatusColor(job: any): string | null {
    const status = getJobStatus(job);
    if (status === 'Complete') return '#4ade80';
    if (status === 'Running' || status === 'Starting') return '#60a5fa';
  ...
  const columns = [
    ...
    { key: 'status', label: 'Status', extract: (c: any) => getJobStatus(c), colorFn: (c: any) => getJobStatusColor(c) },
    { key: 'completions', label: 'Completions', extract: (c: any) => `${c?.status?.succeeded ?? 0}/${c?.spec?.completions ?? 1}` },
    { key: 'active', label: 'Active', extract: (c: any) => String(c?.status?.active ?? 0) },
    { key: 'duration', label: 'Duration', extract: (c: any) => formatDuration(c?.status?.startTime, c?.status?.completionTime) },
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No jobs found in this namespace." hrefFn={(entry) => `/resources/jobs/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
