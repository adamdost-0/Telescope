<script lang="ts">
  ...
  const GVK = 'v1/PersistentVolume';
  const PAGE_TITLE = 'Persistent Volumes';
  ...
  const columns = [
    ...
    { key: 'capacity', label: 'Capacity', extract: (c: any) => c?.spec?.capacity?.storage ?? '' },
    { key: 'access-modes', label: 'Access Modes', extract: (c: any) => (c?.spec?.accessModes ?? []).join(', ') },
    { key: 'reclaim-policy', label: 'Reclaim Policy', extract: (c: any) => c?.spec?.persistentVolumeReclaimPolicy ?? '' },
    { key: 'status', label: 'Status', extract: (c: any) => c?.status?.phase ?? '' },
    { key: 'storage-class', label: 'Storage Class', extract: (c: any) => c?.spec?.storageClassName ?? '' },
    { key: 'claim', label: 'Claim', extract: (c: any) => {
      const ref = c?.spec?.claimRef;
      if (!ref) return '';
...
<div class="resource-page">
  <header>
    <h1>{PAGE_TITLE}</h1>
    <div class="controls">
      <span class="ns-label">Scope: <strong>Cluster</strong></span>
      {#if lastUpdatedText}
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No persistent volumes found." hrefFn={(entry) => `/resources/persistentvolumes/${entry.namespace || '_cluster'}/${entry.name}`} />
  {/if}
</div>
