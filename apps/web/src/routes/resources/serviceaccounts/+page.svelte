<script lang="ts">
  ...
  const columns = [
    ...
    { key: 'secrets', label: 'Secrets', extract: (c: any) => String((c?.secrets ?? []).length) },
    { key: 'wi', label: 'Workload Identity', extract: (c: any) => {
      const clientId = c?.metadata?.annotations?.['azure.workload.identity/client-id'];
      return clientId ? '✓ Azure WI' : '—';
    }},
...
<div class="resource-page">
  ...
  {:else}
    ...
    <ResourceTable resources={filtered} {columns} emptyMessage="No service accounts found in this namespace." hrefFn={(entry) => `/resources/serviceaccounts/${entry.namespace}/${entry.name}`} />
  {/if}
</div>
